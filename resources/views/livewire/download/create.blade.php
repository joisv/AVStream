<x-modals>
    ...
    ">
        <div class="flex justify-between">
            <div>
                <x-primary-button wire:click="addDownload" x-data type="button"
        ...
        <form wire:submit.prevent="save">
            <div class=" p-2 rounded-md space-y-2">
                <x-inputs.label-input for="genres"></x-input.lable-input>
                    <div class="flex space-x-2">
                        <div class="w-11/12">
                            <x-inputs.text id="genres" wire:model="search" placeholder="search title post" autocomplete="off"/>
                            @error('post_id')
                            ...
                            <div class="w-fit px-2 py-1 relative group">
                                <label for="{{ $post->id }}"
                                    ...
                                    <input wire:model="post_id" type="radio" value="{{ $post->id }}"
                                        id="{{ $post->id }}" class="absolute opacity-0">
                                </label>
                            </div>
                        @empty
                            <div
            ...
            @foreach ($downloads as $index => $download)
                <div class="flex items-end space-x-2 w-full" wire:key="download-{{ $index }}">
                    <label for="isVip-{{ $index }}" class="{{ $downloads[$index]['isVip'] ? 'bg-rose-500' : 'bg-gray-400' }} h-fit p-2 relative cursor-pointer rounded-sm">
                        <input id="isVip-{{ $index }}" wire:model="downloads.{{ $index }}.isVip" type="checkbox" name="" class="absolute opacity-0">
                        <x-icons.crown />
                    </label>
                    <div class="space-y-2 w-full">
                        <x-inputs.label-input for="name-{{ $index }}">name</x-input.lable-input>
                            <x-inputs.text-input type="text" id="name-{{ $index }}"
                                wire:model.defer="downloads.{{ $index }}.name" placeholder="HD 720p (2gb)" />
                            @error("downloads.{$index}.name")
                                <span class="error">{{ $message }}</span>
                            @enderror
                    </div>
                    <div class="space-y-2 w-full">
                        <x-inputs.label-input for="embed-{{ $index }}">url download</x-input.lable-input>
                            <x-inputs.text-input type="text" id="embed-{{ $index }}"
                                wire:model.defer="downloads.{{ $index }}.url_download" placeholder="url download" />
                            @error("downloads.{$index}.url_download")
                                <span class="error">{{ $message }}</span>
                            @enderror
                    </div>
                    <div>
                        <x-primary-button :disabled="$index === 0" wire:click="deleteDownload({{ $index }})" x-data
