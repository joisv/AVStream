    ...
    <div class="space-y-4">
        ...
        <div
            ...
            <div class="flex flex-col justify-center items-center bg-gray-900 w-full p-2">
                <h1 class="text-xl font-semibold">Total Download</h1>
                <h4 wire:loaing.remove>{{ $downloadCount }}</h4>
                ...
                <x-slot name="body">
                    @forelse ($downloads as $download)
                        <x-tables.row>
                            ...
                            <x-tables.cell>
                                <x-icons.crown isVip="{{ $download->isVip }}" />
                            </x-tables.cell>
                            <x-tables.cell>
                                {{ $download->user->name ?? '' }}
                            </x-tables.cell>
                            ...
                            <x-tables.cell>
                                <div class="flex space-x-1">
                                    <x-primary-button wire:click="editDownload({{ $download }})" type="button">
                                        <x-slot name="icon">
                                            <x-icons.edit />
                                        </x-slot>
                                    </x-primary-button>
                                    <x-primary-button custom="bg-red-600 hover:bg-red-800 focus:ring-red-300"
                                        wire:click="destroyAlert({{ $download->id }})">
                                        <x-slot name="icon">
                                            <x-icons.delete />
                                        </x-slot>
                                    </x-primary-button>
