        ...
        <form wire:submit.prevent="save">
            ...
            @foreach ($downloads as $index => $download)
                <div class="flex items-end space-x-2 w-full" wire:key="download-{{ $index }}">
                    <label for="isVip_{{ $index }}" class="{{ $downloads[$index]['isVip'] ? 'bg-rose-500' : 'bg-gray-400' }} h-fit p-2 relative cursor-pointer rounded-sm">
                        <input id="isVip_{{ $index }}" wire:model="downloads.{{ $index }}.isVip" type="checkbox" name="" class="absolute opacity-0 z-20">
                        <x-icons.crown />
                    </label>
