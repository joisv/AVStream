    ...
    <div class="space-y-4">
        ...
        <div
            ...
            <div class="flex flex-col justify-center items-center bg-gray-900 w-full p-2">
                <h1 class="text-xl font-semibold">Total User</h1>
                <h4 wire:loaing.remove>{{ $userCount }}</h4>
                <div wire:loading.flex wire:target="getUserCount" class="w-6 h-6 items-center">
                    <x-icons.loading-circle />
                </div>
            </div>
            <div class="flex flex-col justify-center items-center bg-gray-900 w-full p-2">
                <h1 class="text-xl font-semibold">Latest user update</h1>
                <h4 wire:loaing.remove>{{ $userUpdate }}</h4>
                <div wire:loading.flex wire:target="getUserCount" class="w-6 h-6 items-center">
                    <x-icons.loading-circle />
                </div>
            </div>
        ...
        <div>
            <x-tables.table wire:loading.class.delay="opacity-75">
                <x-slot name="head">
                    ...
                    <x-tables.heading sortable wire:click="sortBy('email')" :direction="$sortField === 'email' ? $sortDirection : null">
                        email
                    </x-tables.heading>
                        ...
                        <x-tables.row>
                            ...
                            <x-tables.cell>
                                <div class="flex space-x-1">
                                    <x-primary-button wire:click="editModal( '{{ $user->id }}' )">
                                        <x-slot name="icon">
                                            <x-icons.edit />
                                        </x-slot>
                                    </x-primary-button>
                                    <x-primary-button custom="bg-red-600 hover:bg-red-800 focus:ring-red-300"
                                        wire:click="destroyAlert('{{ $user->id }}')">
                                        <x-slot name="icon">
                                            <x-icons.delete />
                                        </x-slot>
                                    </x-primary-button>
