    ...
    <div class="space-y-4">
        ...
        <div
            ...
            <div class="flex flex-col justify-center items-center bg-gray-900 w-full p-2">
                <h1 class="text-xl font-semibold">Total Payment</h1>
                <h4 wire:loaing.remove>{{ $paymentCount }}</h4>
            ...
            <div class="flex flex-col justify-center items-center bg-gray-900 w-full p-2 space-y-2">
                <h1 class="text-xl font-semibold">Latest payment updated</h1>
                <h4 wire:loaing.remove class="text-center text-sm font-light">
                    {{ Str::limit($paymentUpdate, 50, '...') }}</h4>
                <div wire:loading.flex wire:target="getPostCount" class="w-6 h-6 items-center">
                    <x-icons.loading-circle />
                </div>
            </div>
        ...
        <div class="space-y-4">
            <x-tables.table>
                <x-slot name="head">
                    ...
                    <x-tables.heading sortable wire:click="sortBy('description')" :direction="$sortField === 'description' ? $sortDirection : null">
                        Description
                    </x-tables.heading>
                        ...
                        <x-tables.row>
                            ...
                            <x-tables.cell>
                                <div class="flex space-x-1">
                                    <x-primary-button wire:click="toggleModal({{ $payment }})" type="button">
                                        <x-slot name="icon">
                                            <x-icons.edit />
                                        </x-slot>
                                    </x-primary-button>
                                    <x-primary-button custom="bg-red-600 hover:bg-red-800 focus:ring-red-300"
                                        wire:click="destroyAlert({{ $payment->id }})">
                                        <x-slot name="icon">
                                            <x-icons.delete />
                                        </x-slot>
                                    </x-primary-button>
