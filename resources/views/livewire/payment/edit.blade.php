        ...
        <form wire:submit.prevent="save">
            <div class="space-y-2">
                <x-inputs.label-input for="name">Name</x-input.lable-input>
                    <x-inputs.text-input type="text" id="name" wire:model.defer="name" placeholder="payment method" />
                    @error('name')
                        <span class="error">{{ $message }}</span>
                    @enderror
            </div>
            <div class="space-y-2">
                <x-inputs.label-input for="description">Description</x-text.label-input>
                    <x-inputs.textarea id="description" wire:model.defer="description" placeholder="description here"/>
                    @error('name')
                        <span class="error">{{ $message }}</span>
                    @enderror
            </div>
